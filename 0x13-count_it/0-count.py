#!/usr/bin/python3
"""
Count given words from all hot posts' titles
"""
import requests

BASE_URL = 'http://reddit.com/r/{}/hot.json'


def count_words(subreddit, word_list, titles=[], after=None):
    """ Get all hot posts """
    headers = {'User-agent': 'Unix:0-subs:v1'}
    params = {'limit': 100}
    if isinstance(after, str):
        if after != "END":
            params['after'] = after
        else:
            return count(word_list, titles)
    response = requests.get(BASE_URL.format(subreddit),
                            headers=headers, params=params)
    if response.status_code != 200:
        return None
    data = response.json().get('data', {})
    after = data.get('after', 'END')
    if not after:
        after = "END"
    titles = titles + [post.get('data', {}).get('title')
                       for post in data.get('children', [])]
    return count_words(subreddit, word_list, titles, after)


def count(word_list, titles):
    """Count words"""
    count = {}
    word_list = [ch.lower() for ch in word_list]
    for word in word_list:
        count[word] = 0
    for title in titles:
        for word in word_list:
            for title_word in title.lower().split():
                if title_word == word.lower():
                    count[word] += 1

    count = {k: v for k, v in count.items() if v > 0}
    print_results(word_list, titles, count)


def print_results(word_list, titles, count):
    words = list(count.keys())
    for word in sorted(words,
                       reverse=True, key=lambda k: count[k]):
        print("{}: {}".format(word, count[word]))
